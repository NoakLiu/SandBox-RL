#!/usr/bin/env python3
"""
OASIS Core Integration with vLLM
================================

集成OASIS核心函数的vLLM仿真，使用真实的OASIS agent graph
"""
...
class OASISAgentState:
    ...
    group: str = "NEUTRAL"  # 兼容OASIS的group字段
...
class OASISCoreAgentGraph:
    """OASIS核心Agent Graph"""
    
    def __init__(self):
        self.agents = {}
        self.edges = defaultdict(set)
        self.sandboxes = {}  # belief_type -> OASISSandbox
    ...
    def add_agent(self, agent: OASISAgentState):
        ...
        if agent.belief not in self.sandboxes:
            self.sandboxes[agent.belief] = OASISSandbox(agent.belief, [])
    ...
    def update_agent_belief(self, agent_id: int, new_belief: BeliefType, belief_strength: float = 0.5):
        ...
        agent.group = new_belief.value  # 更新OASIS兼容的group字段
        
        # 添加到新sandbox
        if new_belief not in self.sandboxes:
            self.sandboxes[new_belief] = OASISSandbox(new_belief, [])
...
class OASISCoreVLLMSimulation:
    ...
    def __init__(self, num_agents: int = 50, vllm_url: str = "http://localhost:8001/v1", 
        ...
        self.model_name = model_name
        self.agent_graph = OASISCoreAgentGraph()
        self.history = []
    ...
    async def run_simulation(self, steps: int = 30):
        """运行仿真"""
        logger.info(f"开始运行 {steps} 步OASIS核心vLLM仿真...")
...
async def main():
    ...
    # 保存结果
    simulation.save_results("oasis_core_vllm_simulation_results.json")
